function [t_edges, r_edges] = solve_graph_non_lin(graph_data,freq, input_voltage_amp,cond2gnd)
...
%           struct should have the following fields:
                ...
                %     edge_num= gnumber of edges in the graph
                %     L_arr = a vector of length edge_num storing the
                %     (physical) lengths of the edges.
                %     v_ph_arr =  a vector of length edge_num storing the
                %     phase velocities of the edges
                %     Y_arr = a vector of length edge_num storing the
                %     characteristic admittances of the edges
                %     BC_arr = a vector of length edge_num storing the
                %     boundary condition value of each edges according to
                %     the following convetion:
                        % 1 - set t to 0
                        % 2 - set r to 0
                        % 3 - set t to input_voltage_amp 
                        % 4 - set r to input_voltage_amp
...
% input_voltage_amp: see BC_arr above.
% cond2gnd: conductivity to the ground (loss) in 1/Ohm. default valte: 0.
% 
% output arguments:
...
for i=1:nodes_num
    ...
    % edges parameters
    % by convention we only add exp(+-ikL) for the inedges. so we set
...
%     Y_out  = Y_arr(outedges);
    ...
    Y_out = sqrt((G-1i*omega*Y_arr(outedges)./v_ph_arr(outedges))./(R - 1i*omega./Y_arr(outedges)./v_ph_arr(outedges)));
         
     k_in=1i*sqrt((R-1i*omega./v_ph_arr(inedges)./Y_arr(inedges)).*(G-1i*omega*Y_arr(inedges)./v_ph_arr(inedges))); % effective complex wave number for primary lines (rad/m)
     k_in = k_in.*sign(real(k_in)); % becuase the sqrt() makes a choice and we want the one where real part of k to be positive. see pdf.
   % %     k_in = omega*(v_ph_arr(inedges)).^-1;
    
   Y_in = sqrt((G-1i*omega*Y_arr(inedges)./v_ph_arr(inedges))./(R - 1i*omega./Y_arr(inedges)./v_ph_arr(inedges))); 
   %Y_in = Y_arr(inedges);
...
for i = 1:length(indices)
    ...
    switch BC_arr(indices(i))
        ...
        case 3
            mat(eqn_count+1, tIdx(ii))=1;
            vec(eqn_count+1)=input_voltage_amp;
            eqn_count = eqn_count+1;
        case 4
            mat(eqn_count+1, rIdx(ii))=1;
            vec(eqn_count+1)=input_voltage_amp;

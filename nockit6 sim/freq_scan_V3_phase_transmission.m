...
% network geometry
    ...
    M = 7; % number of lines
    idx_of_input_lines =line; % either an integer in [1,M]  or an array of integers in [1,M]
...
figure(4)
clf
    plot(Frequency, trans_phase(:,line)', 'linewidth', 1.5)
    grid on
    ...
    P = polyfit(1e-9*Frequency, trans_phase(:,line)',1 );
    yfit = 1e-9*P(1)*Frequency +P(2);
    slope = 1e-9*P(1);
    v_ph_est = 2*pi*L*N/slope;
    
    hold on
    plot(Frequency, yfit, '--')
    leg = legend([sprintf('%d-->%d', line,line), sprintf("linear fit, slope = %.3g\n v_p_h_,_e_s_t = %.4g", slope, 2*pi*L*N/slope)],"location", "best", "fontsize", 16);
